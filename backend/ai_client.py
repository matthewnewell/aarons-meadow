...
def _depot(messages: list[dict], system: str, max_tokens: int = 1024) -> str:
    ...
    AI_API_KEY of its own, just DEPOT_API_URL, which depot_client.py already reads for its
    person-name lookups — imported from there rather than re-declared here, so the two stay in
    sync automatically. Same error/degrade shape as every other provider function: a string
    starting "[AI error: ...]" or a plain reachability message, never an exception."""
    try:
        import httpx
    except ImportError:
        return "httpx package not installed. Run: pip install httpx"
